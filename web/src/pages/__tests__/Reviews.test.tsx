...
const mockFetchReviews = fetchReviews as MockedFunction<typeof fetchReviews>;
...
describe("ReviewsPage", () => {
  ...
  describe("error state", () => {
    it("shows error message when failing to get data", async () => {
      const consoleSpy = vi.spyOn(console, "error").mockImplementation(() => {}); // avoid console error
      // Arrange
  ...
  describe("success state", () => {
    it("renders fetched data", async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(`App ID: test-app-123 • 2 reviews (${DEFAULT_TIME_RANGE}h)`)).toBeTruthy();
  ...
  describe("filter functionality", () => {
    it("filters reviews by rating", async () => {
      ...
      // Trigger filter by clicking the filter button
    ...
    it("filters reviews by date range", async () => {
      ...
      act(() => {
        fireEvent.click(timeRangeDropdownTrigger);
      });
      ...
      act(() => {
        fireEvent.click(timeRange72Option);
      });
    ...
    it("combines rating and time range filters", async () => {
      ...
      act(() => {
        fireEvent.click(timeRange24Option);
      });
